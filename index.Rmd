...
* Halp! In PS2 I got a negative house price predictions $\widehat{y}$ using, for example, `OverallQual` as a predictor! What do I do?
* Go over solutions to Exercise from Lec09 on showing consequences (on RMSLE) of making predictions on separate `test` data using a model that is HELLA overfit to the `training` data.
* 
...
1. CART wrap-up
    ...
    a) Tie-in CART "complexity parameter" $\alpha$ with Lec01 "What is Machine Learning?" [slide 20](static/what_is_ML.pdf#page=20){target="_blank"}.
