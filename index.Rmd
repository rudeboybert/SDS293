...
1. PS9 lite on "linear regression via linear algebra" is now posted in the GitHub organization for this course. This is the final problem set for the course. Because it is *very* short, you'll be working individually.
1. Book club readings posted in the schedule above. For example Monday's readings are Chapters 1-2. By Monday 9:30am EDT I will post a brief video summarizing my thoughts on the readings. We can then discuss the readings in `#questions` on Slack.
...
Last one for the semester! Watch this brief screencast on two types of clustering:

1. Dendrograms: See this [PDF](static/tree_dendrogram.pdf){target="_blank"} of a dendrogram of tree species
1. k-means clustering: See this [PDF](static/methods/k_means/k_means.pdf){target="_blank"}

<center>
<iframe src="https://www.showme.com/sma/embed/?s=mrOOisS" width="578" height="433" frameborder="0"></iframe>
</center>
...
1. Today is your last chance to let me know if our plan for Midterm II this Friday does not work for you. See [Midterm II](#midtermII) for more information.
1. I will post a plan for our "Deep Learning" book club on Friday. See [Lec27](#bookclub) for more information.


## Chalk Talk

1. First watch the screencast on "simple linear regression as matrix algebra" I've posted below.
1. Then go over the `regression_linear_algebra.Rmd` file I will distribute on Slack. Here you will study the effects of **collinearity** in regression (which you learned in SDS 291 Multiple Regression), but from a linear algebra perspective.

<center>
<iframe src="https://www.showme.com/sma/embed/?s=12ejGF6" width="578" height="433" frameborder="0"></iframe>
</center>
...
## Announcements {#bookclub}

1. See Midterm II details below.
1. PS10 lite on Deep learning/neural nets is now cancelled. So after you submit PS9 lite on Fri 4/24 at 9pm EDT, you have completed all evaluative components of this course. 
1. The last two weeks will be an optional **book club** on [Deep Learning Illustrated](https://www.deeplearningillustrated.com/){target="_blank"} (see [Lec24](#Lec24) for how to obtain free access to the eBook). Please let me know if you have ideas for the format.
1. Before being thrown into a massive task, I always find it helpful to set some goals and define a purpose. This helps me stay focused and not get lost in the weeds:
    1. **Micro goal of book club**: Understand to what researchers mean when they say deep learning is a ["black box"](https://towardsdatascience.com/the-black-box-metaphor-in-machine-learning-4e57a3a1d2b0){target="_blank"} where nobody really understands how these algorithms work. Prof. Safiya Umoja Noble stated in the podcast on "Algorithms of Oppression" from [Lec18](#Lec18) that this lack of interpretability will be problematic for society as more of our lives becomes dependent on such algorithms.
    1. **Macro goal of book club**: To develop you into ambassadors in your communities who can explain to laypeople roughly how these algorithms work and what impact they can have if developed and implemented recklessly.

<center>
![](static/images/deep_learning_illustrated.jpg){ width=30% }
</center>
...
* Time window:
    ...
    + You must submit your work on Moodle before ~~Tue 4/21~~ Wed 4/22 11pm EDT
* What you will need:
...
* Here are three ways to convert your responses to electronic format for submission on Moodle:
    1. If you have one, use a scanner to create a PDF. I'm assuming that most of you don't have one.
    1. Use the [CamScanner app](https://www.camscanner.com/user/download){target="_blank"} on your phones to create PDF's. I'm assuming that this will work for most of you. Note: they will be pushing you to purchase the premium version, but you don't need to buy it. Using the freemium version will suffice. 
    1. At last resort, take pictures with your phone and submit those.


### b) Topics covered:

1. You will be given an optional opportunity to re-do Midterm I Question 2. It is important for *everyone* in this course to master the themes in this question in particular. Feel free to discuss with your peers.
1. Some form of the other questions in Midterm I may re-appear in Midterm II, so I suggest you go over Midterm I.
1. LASSO Regularization: Lec 19-24. In particular, you should compare-and-contrast LASSO with CART. While the details of these two methods differ slightly, they are almost identical in spirit.
1. Random Forests: Lec 25-26. 
1. PS7 on LASSO: Please save the `PS7_solutions.Rmd` file I will distribute over Slack in your `PS7` RStudio Project folder and knit it.
...
1. Note that: "Unsupervised learning" are machine learning methods where there is no outcome variable $y$. Two of the main methods for unsupervised learning are:
    1. Principal Component Analysis (PCA): A method for dimension reduction of your $p$ predictor variables to a smaller number.
    1. Cluster analysis: Assigning observations to "clusters" whereby *within* cluster differences are minimized, but *between* cluster differences are maximized. 
1. Download `PCA.Rmd` and `sat.csv` from Slack and put them in your MassMutual RStudio Project and go over `PCA.Rmd`
1. Note that conveying all the theory is very difficult without doing an actual "chalk talk." So please try to focus on the ideas in a general and qualitative sense and not get bogged down in the details:
    1. A **basis**: A set of vectors that **span** a space. In this case, some combination of the blue and red vectors (i.e. arrows) allow you to "reach" anywhere in the 2D space.
    1. An **orthogonal basis**: A basis where the vectors are perpendicular, i.e. form a 90 degree angle, i.e. have a dot product of 0. 
    1. PCA as a **linear transformation** as represented by a matrix. In this case, the transformation is a "rotation."
    1. "Eigen" is German for "its own." In this case an **eigenvalue** is a number telling you how much variance there is in the data along a particular dimension. The particular dimension for a particular eigenvalue is the **eigenvector**.
...
1. PS8 lite is now posted in the GitHub organization for this course:
    1. This PS is very quick: just fit ONE random forests model using any approach of your choice.
    1. However, the submission format is a little different. You will be "forking" a copy of PS8 lite and then making a "GitHub Pull Request." Please watch this 6min video on how before you start working:
...
1. Recorded screencast of PS3 solutions: picking the "optimal" complexity paramemter $\alpha$ via cross-validation

<iframe width="560" height="315" src="https://www.youtube.com/embed/-SQEbwfRVPA" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
