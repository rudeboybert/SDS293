...
* Why is it important to work in "RStudio Project" mode? Because of how you set your working directory using relative vs absolute file paths; see tweet below.
...
* Introducing the `#mp1` channel in Slack for questions on mini-project 1.
* Clarifications on what external sources you can use. 
...
Shared with me by Dr. Smetzer. I agree with Ms. Damour's opinion; you don't have to, but her ideas are still worth thinking about. Relating to "points of diminishing returns" and "polishing the cannonball" as discussed for mini-project 1, here are some excerpts:

* Overqualified and overprepared, too many _insert whatever minority group_ still hold back. _Insert whatever minority group_ feel confident only when they are perfect.
* First, parents and teachers can stop praising inefficient overwork, even if it results in good grades.
* A colleague of mine likes to remind teenagers that in classes where any score above 90 counts as an A, the different between a 91 and a 99 is a life.
...
plot1 <- overall %>% 
  filter(df == 2) %>% 
  ggplot(aes(x=x)) +
  geom_point(aes(y=y), size=0.75) +
  geom_line(aes(y=.fitted), col="blue", size=0.5) +
  labs(title="Plot 1: Splines with df = 2")
...
plot2 <- overall %>% 
  ...
  geom_point(aes(y=y), size=0.75) +
  geom_line(aes(y=.fitted), col="blue", size=0.5) +
  labs(title="Plot 2: Splines with df = 25")
...
plot3 <- overall %>% 
  ...
  geom_point(aes(y=y), size=0.75) +
  geom_line(aes(y=.fitted), col="blue", size=0.5) +
  labs(title="Plot 3: Splines with df = 50")

plot4 <- overall %>% 
  filter(df == 50) %>% 
  ggplot(aes(x=x)) +
  geom_line(aes(y=y), col="blue", size=0.5) +
  geom_point(aes(y=y), size=0.75) +
  labs(title="Plot 4: Extreme case")
