...
Let's now consider three models for credit card `Balance`
...
The mean credit card balance AKA credit card debt $\bar{y}$ is:

```{r}
mean(credit$Balance)
```

thus a reasonable guess $\widehat{y}$ of an individual's credit bard balance using no predictor information would be $\bar{y}$ = $520.01. Let' visualize this in a histogram:

```{r, fig.width=8}
ggplot(credit, aes(x = Balance)) +
  geom_histogram(binwidth = 100, boundary = 0) +
  labs(x = "Balance ($)", title = "Histogram of outcome variable: credit card balance") +
  geom_vline(xintercept = mean(credit$Balance), col = "red", size = 1)
```

<!--
Let's also visualize this in an interactive 3D plot:
-->
...
In other words, think of the following fitted model $\widehat{f}(\vec{x})$ as a **minimally viable model** AKA a "null" model AKA a "most basic simplest baseline model".
...
But what are the fitted coefficients/parameters $\widehat{\beta}_0$ and $\widehat{\beta}_0$? You can read them from the estimate column from the regression table below, which we generated using the `get_regression_table()` "wrapper" function to extract regression tables as a data frame:
...
\widehat{y} &= \widehat{\beta}_0 + \widehat{\beta}_1x_1\\
\widehat{\mbox{Balance}} &= \widehat{\beta}_0 + \widehat{\beta}_1\mbox{Income}\\
\widehat{\mbox{Balance}} &= 246.515 + 6.048 \mbox{Income}
\end{aligned}
$$
...
\widehat{\mbox{Balance}} &= -385.179 - 7.663 \mbox{Income} + 0.264\mbox{Limit}\\
\end{aligned}
$$
...
Let’s visualize all three models, with
