...
* G = ["Gini impurity"](https://en.wikipedia.org/wiki/Decision_tree_learning#Gini_impurity) is a measure of "fit" where smaller values indicate better fit (see [ISLR](http://faculty.marshall.usc.edu/gareth-james/ISL/) p.312).
* $\alpha$ here is the "complexity parameter", which you can set using the `cp` argument in `rpart.control()`.

For example explicitly setting $\alpha$ = `cp=0.01` (which is the default value; see `?rpart.control`), the code is:

```{r, eval=FALSE}
# Define tree
tree_parameters <- rpart.control(cp = 0.01)
model_CART <- rpart(Species ~ Sepal.Length + Sepal.Width, 
                    data = iris, 
...
renderPlot({
  ...
  model_CART <- rpart(Species ~ Sepal.Length + Sepal.Width, data = iris, control=tree_parameters)
